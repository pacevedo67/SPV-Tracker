...
- **Step 3 (shipped)** — cross-firm portability: `certifications` and `certification_access_grants` tables; `POST /api/submissions/:id/sign` mints a certification, supersedes prior active certs for the account, carries forward active grants, and auto-grants the inviting firm. New invitation type `access_request` (with new columns `invitations.type` and `invitations.certification_id`); firm calls `POST /api/matters/:id/request-access`, investor approves at `/investor-certifications.html`. `GET /api/submissions/:id` allows reads from firms with a live grant on the cert. `POST /api/contacts/lookup-cert` powers the matter UI's "Request Portable Access" affordance.
- **Step 4 (shipped)** — certification expiry and renewal: certs default to `certified_at + 1 year`, a startup-scheduled `runCertExpirySweep()` (5s after boot, then every 24h) marks past-due certs `expired` and emails the account admin once per cert when expiry is within 30 days (gated by `certifications.expiry_reminder_sent_at`), `matter.html` shows ⚠ Expiring/Expired badges, and investors can self-renew via `POST /api/investor/certifications/:id/renew` (clones the prior submission into a new draft, stages a guest session, signing supersedes the old cert and carries forward grants).
